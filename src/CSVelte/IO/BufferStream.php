...
class BufferStream implements Streamable
{
     ...
     *
     * Instantiate a new buffer stream, optionally changing the maximum buffer
     * size from its default of 16384 bytes (16KB)
     *
     * @param \CSVelte\IO\Resource A stream resource object
     */
    public function __construct($maxBufferSize = null)
    {
        if (!is_null($maxBufferSize)) {
            $this->meta['maxBufferSize'] = $maxBufferSize;
        }
    }

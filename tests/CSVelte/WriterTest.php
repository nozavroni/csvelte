...
{
    protected $testdata = [
        ['id', 'title', 'summary', 'address', 'city', 'state', 'zip', 'notes'],
        ['1', 'This is a test title', '    And a summary that you want to read because it is so summarrific', '123 Address St.', 'Cityville', 'ST', '12345', 'Notes are for jerks and losers'],
        ['2', 'This has, a, comma or two', 'isn\'t this apostrophe pretty?', '321 Nough Rd', 'Nonestown', 'NO', '54321', '   Notes are the best place to put "text containing quotes" or even quotes containing "text".     '],
        ['3', "I\tlike ham soda", 'I\'m a silly little summary', '555 Silly Avenue', 'Eden', 'CA', '55651', 'These; ~notes~ `cont@in, character$: _that_ *are* /sometimes/ \'used\' -in- |p|ace| \\of\\ <commas> [when] {writing} #CSV %data.'],
        ['4', 'I\'m a "title"', 'Summarize <strong>this</strong>', '87-845 Something; cool Drive', 'Coolsville', 'CT', '68452-4257', 'These notes contain no special characters at all not even a period'],
        ['5', 'This is the title of it', "A summary isn't to be taken lightly", '1122 Some Rd Apt #12-A', 'The Town', 'PP', '12223', "I decided to \n put a bunch of \r random \r\n\r\nline\nterminators in this notes\r\nfield. Weird, huh?"],
    ];
    ...
    public function testWriterCustomFlavor()
    {
        ...
        $writer = new Writer($out, $expectedFlavor = new Flavor(['delimiter' => '|']));
    ...
    public function testWriterWriteHeaderRow()
    {
        ...
        $writer = new Writer($out, $flavor = new Flavor([
            'header'         => true,
            'doubleQuote'    => true,
            'lineTerminator' => "\n",
        ]));
    ...
    public function testWriterThrowsExceptionIfUserAttemptsToSetHeaderAfterRowsHaveBeenWritten()
    {
        ...
        $writer->setHeaderRow(['this', 'shouldnt', 'work']);
    }
    ...
    {
        $flavor = new Flavor(['lineTerminator' => "\r\n"]);
        $out = new Stream('php://memory');
    ...
    {
        $reader = CSVelte::reader(__DIR__.'/../files/banklist.csv', $flavor = new Flavor([
            'header' => true,
        ]));

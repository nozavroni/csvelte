...
class ReaderTest extends TestCase
{
    ...
    {
        $flavor = new Flavor(['quoteStyle' => Flavor::QUOTE_MINIMAL, 'lineTerminator' => "\n"], ['hasHeader' => false]);
    ...
    {
        $flavor = new Flavor(['header' => false]);
        ...
        $this->assertEquals($expected = ['1', 'Eldon Base for stackable storage shelf, platinum', 'Muhammed MacIntyre', '3', '-213.25', '38.94', '35', 'Nunavut', 'Storage & Organization', '0.8'], $reader->current()->toArray());
    }
    ...
    public function testReaderNext()
    {
        ...
        $reader = new Reader(new Stream(realpath(__DIR__.'/../files/SampleCSVFile_2kb.csv')), $flavor);
        $this->assertEquals($expected = ['1', 'Eldon Base for stackable storage shelf, platinum', 'Muhammed MacIntyre', '3', '-213.25', '38.94', '35', 'Nunavut', 'Storage & Organization', '0.8'], $reader->current()->toArray());
        $this->assertEquals($expected = ['2', '1.7 Cubic Foot Compact "Cube" Office Refrigerators', 'Barry French', '293', '457.81', '208.16', '68.02', 'Nunavut', 'Appliances', '0.58'], $reader->next()->toArray());
        $this->assertEquals($expected = ['2', '1.7 Cubic Foot Compact "Cube" Office Refrigerators', 'Barry French', '293', '457.81', '208.16', '68.02', 'Nunavut', 'Appliances', '0.58'], $reader->current()->toArray());
    }
    ...
    {
        $flavor = new Flavor(['header' => true]);
        $reader = new Reader(new Stream(realpath(__DIR__.'/../files/banklist.csv')), $flavor);
        $this->assertEquals(
            $expectedHeader = ['Bank Name', 'City', 'ST', 'CERT', 'Acquiring Institution', 'Closing Date', 'Updated Date'],
    ...
    public function testHeaderRowIsAlwaysSkippedWhenWorkingWithReader()
    {
        ...
        $this->assertEquals($expectedFirstRow = ['Bank Name' => 'First CornerStone Bank', 'City' => 'King of Prussia', 'ST' => 'PA', 'CERT' => '35312', 'Acquiring Institution' => 'First-Citizens Bank & Trust Company', 'Closing Date' => '6-May-16', 'Updated Date' => '25-May-16'], $reader->current()->toArray());
        $this->assertEquals($expectedHeader = ['Bank Name', 'City', 'ST', 'CERT', 'Acquiring Institution', 'Closing Date', 'Updated Date'], $reader->header()->toArray());
        // make sure that running through foreach starts with row #2
        foreach ($reader as $line_no => $row) {
            $this->assertEquals(2, $line_no);
            $this->assertEquals($expectedFirstRow = ['Bank Name' => 'First CornerStone Bank', 'City' => 'King of Prussia', 'ST' => 'PA', 'CERT' => '35312', 'Acquiring Institution' => 'First-Citizens Bank & Trust Company', 'Closing Date' => '6-May-16', 'Updated Date' => '25-May-16'], $row->toArray());
            break;
        }
    }
    ...
    public function testReaderStripsQuotesFromQuotedCells()
    {
        ...
        $reader = new Reader($in, new Flavor(['lineTerminator' => "\n"]));
...
';
        $reader = CSVelte::stringReader($data, new Flavor(['lineTerminator' => '
', 'header'                                                                 => true]));
        $i = 0;
        foreach ($reader as $row) {
            $i++;
